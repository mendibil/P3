...
void print_results(int nframes, int num_voiced, int num_unvoiced,
  ...
  /*
  cout << endl << "Num. frames:\t" << nframes 
       << " = " << num_unvoiced << " unvoiced + " 
       << num_voiced << " voiced\n";
  */
  // cout << endl << "Unvoiced frames as voiced:\t" << num_unvoiced_voiced << "/" << num_unvoiced 
  ...
  */
  ...
  ///cout << fixed << setprecision(2);
  ///cout << "\n===>\t" << filename << ":\t" << 100 * score << " %\n";
  cout /*<< "===>\t" */<< 100* 10000 * score ;//<< " %\n";

  //Afegit per mi
  ofstream fitxer;
  fitxer.open("iteracions.txt", std::ios_base::app);
  fitxer << "===>\t" << 100 * score << " %\n";
  fitxer.close();
}
